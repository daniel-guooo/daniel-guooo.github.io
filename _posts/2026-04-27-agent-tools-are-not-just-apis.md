...
1. The goal of tool design is not to force the agent into one correct path.
...
1. Not every interface that looks reasonable in traditional software is a good interface for an agent.
...
1. You cannot judge tool quality by intuition alone.
...
1. Evaluation should not over-specify the agent's path.
...
1. Accuracy is not the only metric that matters.
...
1. A good tool should save the agent's context instead of exposing all the complexity of the underlying system.
...
1. The unit of tool design should be the workflow, not the API endpoint.
...
1. A tool response should not aim to be as complete as possible.
...
1. Agents are much better with natural-language semantics than with cryptic identifiers.
...
1. A tool can use a `response_format` option to let the agent choose between `concise` and `detailed` responses.
...
1. Error responses need design too.
...
1. If a tool can return a large amount of content, it should be built from the beginning with ways to fetch less, fetch precisely, and fetch on demand.
...
1. Tool descriptions, parameter names, and schemas are prompt engineering.
