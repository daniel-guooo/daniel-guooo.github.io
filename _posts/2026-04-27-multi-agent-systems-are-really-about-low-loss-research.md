...
1. Search is not retrieval. Search is compression.
...
1. Multi-agent systems work partly because they spend more effective tokens.
...
1. Multi-agent systems are only worth it for high-value tasks.
...
1. Not every complex task is a good multi-agent task.
...
1. The hard part is orchestration, not spawning subagents.
...
1. The common failure modes are very concrete.
...
1. A good research agent starts wide, then narrows.
...
1. Prompting should teach heuristics, not just hard rules.
...
1. Evaluation has to change when the path is not deterministic.
...
1. LLM-as-judge is useful, but it does not remove the need for human testing.
...
1. Production reliability is about state, not just intelligence.
...
1. Memory is not just about remembering more.
...
1. Synchronous execution is simpler, but it blocks information flow.
...
1. Asynchronous execution is more powerful, but it makes coordination much harder.
...
1. The broader takeaway is parallel task design.
